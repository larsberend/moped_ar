...
def warp_img(src, H, angle=None, inv=True, yellow=None, blue=None, dst_height=4000, dst_width=4000):
    ...
    if inv == True:
        ...
        # dst_points = my_interpol(dst_points)
...
def my_interpol(img):
    ...
    # mask = np.full((img.shape[0], img.shape[1]), fill_value=255)
    imshape = img.shape

    nonzero = np.where(np.all(img!=[0,0,0], axis=-1))

    top_left = [0,0]

    top_right_idx = np.unravel_index(np.argmax(nonzero[1], axis=None), nonzero[1].shape)
    top_right = [nonzero[1][top_right_idx], nonzero[0][top_right_idx]]

    # lower_left_idx = np.unravel_index(np.argmax(nonzero[1], axis=None), nonzero[1].shape)
    # lower_left = [nonzero[0][top_right_idx], nonzero[1][top_right_idx]]

    lower_right_idx = np.unravel_index(np.argmax(nonzero[0], axis=None), nonzero[1].shape)
    lower_right = [nonzero[1][lower_right_idx], nonzero[0][lower_right_idx]]
    print(lower_right)
    ...
    cv.imwrite('./roi_bv.png', roi_image)

    zero_roi = np.where(np.all(roi_image==[0,0,0], axis=-1))

    mask[zero_roi] = [0,0,0]

    cv.imwrite('zero_roi.png', mask)
    mask = mask[:,:,0]


    # mask = region_of_interest(mask, vertices)
    # cv.imwrite('./roi_mask.png', mask)
    #
    # mask = mask[:,:,0]
    #
    # mask[nonzero_roi] =

    inpaint = cv.inpaint(img, mask, 5, cv.INPAINT_TELEA)
    cv.imwrite('./inpainted.png', inpaint)
    quit()


def my_interpol2(img):
    img = cv.resize(img, (500,500))
    zero = np.where(np.all(img==[0,0,0], axis=-1))
    interpolated = np.zeros_like(img)
    # print(zero)
    for i in np.arange(zero[0].size):
        print((i, zero[0].size))
        j = zero[0][i]
        dist1, dist2, nonzero1, nonzero2 = None,None,None,None
        while(j<img.shape[0]):
            if np.all(img[zero[0][i],j] != [0,0,0]):
                dist1 = np.abs(j-zero[0][i])
                nonzero1 = img[zero[0][i],j]
                break
            else:
                j = j+1
        j = zero[0][i]
        while(j>0):
            if np.all(img[zero[0][i],j] != [0,0,0]):
                dist2 = np.abs(j-zero[0][i])
                nonzero2=img[zero[0][i],j]
                break
            else:
                j = j-1
        ...
        if dist1 and dist2:
            ...
            value = nonzero1 * dist1_norm + nonzero2 * dist2_norm

            interpolated[zero[0][i], zero[1][i]] = value
    cv.imwrite('interpol.png',interpolated)
    quit()
    return interpolated
        #
        # nonzero = np.where(np.all(img[:,i]!=[0,0,0]), axis=-1)
        # if nonzero.size > 0:
        #     dist1 = nonzero[0][1]
